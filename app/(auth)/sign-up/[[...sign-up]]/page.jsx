...
export default function SignUpPage() {
  ...
  const handleComplete = async (result) => {
    if (result?.createdUserId) {
      try {
        // Stockage immédiat du rôle dans localStorage
        ...
        console.log("Rôle sélectionné avant API:", selectedRole);

        // Envoi du rôle sélectionné à notre API avec retry
        let apiSuccess = false;
        for (let attempt = 0; attempt < 3 && !apiSuccess; attempt++) {
          try {
            ...
            if (response.ok) {
              apiSuccess = true;
              console.log("Réponse API update-user-role:", data);
            } else {
              throw new Error(
            ...
            // Si erreur, pause avant nouvelle tentative
            if (!apiSuccess && attempt < 2) {
          ...
          <div className="max-w-lg sm:max-w-xl md:max-w-2xl lg:max-w-3xl w-full">
            ...
            <div className="mb-6 px-6">
              ...
              <div className="flex space-x-4">
                <label className="flex items-center cursor-pointer bg-gray-50 p-3 rounded-md border border-gray-200 hover:bg-gray-100 transition">
                  <input
                    type="radio"
                    name="role"
                    value="user"
