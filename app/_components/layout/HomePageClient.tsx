 ...
 */
export default function HomePageClient({ listings }: HomePageClientProps): JSX.Element {
  const [viewMap, setViewMap] = useState(false);

  const handleViewMap = useCallback(() => setViewMap(true), []);

  return viewMap ? <Explore listings={listings} /> : <HomeContent onViewMap={handleViewMap} />;
}
