...
function ExploreNearby() {
  const { t } = useLanguage(); // ✅ Utilisation correcte du `t`

  const cards = [
    {
      title: t("Marchés Biologiques"),
      description: t(
        "Découvrez les marchés biologiques et les événements saisonniers près de chez vous."
      ),
      badge: t("Featured"),
      badgeColor: "text-teal-600",
    },
    {
      title: t("Produits Locaux"),
      description: t(
...
// ✅ ExploreCard reçoit maintenant `t` comme prop
function ExploreCard({ title, description, badge, badgeColor, t }) {
  return (
    <div className="relative overflow-hidden rounded-3xl shadow-md hover:shadow-lg transition-all duration-300 bg-gradient-to-br from-[#F9F7F1] to-[#EAEAEA] h-72 flex flex-col justify-between p-6 md:p-8">
      ...
      <button className="mt-auto px-5 py-2 bg-green-600 hover:bg-green-700 text-white rounded-full shadow-sm transition-colors flex items-center gap-2 text-sm group-hover:gap-3 duration-300">
        {t("discover")}
        <svg
