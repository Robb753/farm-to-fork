...
/**
 * Couleurs (évite Tailwind dynamique bg-${color}-100)
 */
type ServiceColor =
  | "green"
  | "blue"
...
const COLOR_BG_100: Record<ServiceColor, string> = {
  green: "bg-green-100",
  blue: "bg-blue-100",
...
const BADGE_COLOR_CLASSES: Record<ServiceColor, string> = {
...
/**
 * Helpers stables (hors composant) -> exhaustive-deps + React Compiler OK
 */
function getCategoryName(categoryId: ServiceCategoryId): string {
  const names: Record<ServiceCategoryId, string> = {
...
function getCategoryDescription(categoryId: ServiceCategoryId): string {
  const descriptions: Record<ServiceCategoryId, string> = {
...
function getCategoryIcon(categoryId: ServiceCategoryId): React.ReactNode {
  const icons: Record<ServiceCategoryId, React.ReactNode> = {
...
function getCategoryColor(categoryId: ServiceCategoryId): ServiceColor {
  const colors: Record<ServiceCategoryId, ServiceColor> = {
    visit: "green",
    workshop: "orange",
    delivery: "blue",
    tasting: "purple",
...
function groupServicesByCategory(services: Service[]): ServiceCategory[] {
  const categoryMap: Partial<Record<ServiceCategoryId, ServiceCategory>> = {};
  ...
  return Object.values(categoryMap) as ServiceCategory[];
}
...
function createServiceFromType(serviceType: string): Service {
  ...
  const base = serviceMap[serviceType] ?? {
    name: serviceType,
  ...
  return {
    id: serviceType,
    bookingRequired: base.category !== "delivery",
    featured: false,
    capacity: 15,
...
export default function ServicesTab({
  ...
  /**
   * Parse et enrichit les données de services (memo OK, deps OK)
   */

const listingId = listing?.id ?? null;
const listingType = listing?.typeferme ?? null;
const rawServices = listing?.additional_services ?? null;
const serviceCategories: ServiceCategory[] = useMemo(() => {
  // si pas de listing => rien
  if (!listingId) return [];

  // listing minimal pour generateDefaultServices
  // (on ne passe que ce que la fonction utilise)
  const listingForDefaults = {
    id: listingId,
    typeferme: listingType,
  } as ListingWithServices;

  if (!rawServices) {
    return generateDefaultServices(listingForDefaults);
  }
  ...
  const enriched = parsedServices.map(createServiceFromType);
  return groupServicesByCategory(enriched);
}, [listingId, listingType, rawServices]);

  /**
   * Filtre les catégories selon la sélection
   */
  const filteredCategories = useMemo(() => {
    if (selectedCategory === "all") return serviceCategories;
  ...
  /**
   * (optionnel) classes de badges selon couleur
   */
  const getColorClasses = useCallback((color: ServiceColor) => {
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-gray-600 text-sm mt-1">
              ...
              {totalServices > 1 ? "s" : ""} dans {serviceCategories.length}{" "}
              catégorie
        ...
        {filteredCategories.map((category) => (
          <div key={category.id} className="space-y-4">
            ...
            <div className="flex items-center gap-3 pb-2 border-b border-gray-100">
              <div
                className={cn("p-2 rounded-lg", COLOR_BG_100[category.color])}
              >
                {category.icon}
              </div>
              ...
              {category.services.map((service) => (
                ...
                >
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {/* (Optionnel) TODO: showBookingForm */}
                    {showBookingForm === service.id ? null : null}
                  </CardContent>
                </Card>
              ))}
            </div>
