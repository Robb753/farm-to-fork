...
/**
 * Fonction utilitaire pour vérifier si un utilisateur peut changer vers un rôle
 */
export function canChangeToRole(
  ...
  if (isAdmin) return true; // Les admins peuvent tout faire

  // Users peuvent devenir farmers
  if (currentRole === "user" && targetRole === "farmer") return true;

  // Farmers peuvent redevenir users
  if (currentRole === "farmer" && targetRole === "user") return true;

  // Personne ne peut devenir admin sans être admin
  return false;
}
 ...
 */
export { VALID_ROLES, MAX_REASON_LENGTH };
