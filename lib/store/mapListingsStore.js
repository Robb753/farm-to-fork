  ...
  devtools(
    persist(
      immer((set, get) => ({
        ...
        toggleFilter: (filterKey, value) =>
          set((state) => {
            ...
            state.filters[filterKey] = updatedValues;
        ...
        resetFilters: () =>
          set((state) => {
            ...
            state.listings.filtered = state.listings.all;
            state.listings.visible = state.listings.all;
          }),
        ...
        fetchListings: async ({
          ...
          forceRefresh = false,
          bounds = null,
          bbox = null, // ✅ Ajout du paramètre bbox pour compatibilité
        } = {}) => {
          const state = get();
          ...
          // ✅ Validation de la page - évite les offsets trop grands
          if (page < 1) page = 1;
          if (!append && page > 1) page = 1; // Force page 1 si pas d'append
          ...
          try {
            ...
            if (activeBounds) {
              // ✅ Gestion flexible des formats de bounds
            ...
            // Gestion spécifique de l'erreur de pagination
            if (error?.code === "PGRST103") {
              // Offset trop grand, il n'y a plus de données
              set((draft) => {
      ...
      })),
      {
        ...
        partialize: (state) => ({
          // Persister seulement les coordonnées et filtres
          map: {
...
export const useListingsActions = () =>
  useMapListingsStore((state) => ({
    ...
    resetListings: state.resetListings, // ✅ Ajout de resetListings
...
export const useFiltersActions = () =>
  useMapListingsStore((state) => ({
    ...
    resetListings: state.resetListings, // ✅ Ajout de resetListings aux filtres aussi
  }));
