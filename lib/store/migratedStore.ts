...
/** ───────────────────────────────────────────────────────────────────────────
 *  Ré-exports attendus par tes composants
 *  (évite les erreurs "not exported from '@/lib/store/migratedStore'")
 *  ─────────────────────────────────────────────────────────────────────────── */
export { MAPBOX_CONFIG } from "@/lib/config/map.config";
export { FILTER_SECTIONS as filterSections } from "@/lib/config/constants";
    ...
    subscribeWithSelector(
      persist(
        (set, get) => ({
          ...
          // ✅ FETCH avec gestion propre du hasMore
          fetchListings: async (options = {}) => {
            ...
            try {
              ...
              // 🔁 hasMore fiable basé sur le total
              ...
              // Gestion "défensive" PGRST103 côté client si ça remonte
          ...
          cleanupPagination: () => {
            // no-op pour l’instant (hook placeholder si besoin)
          },
